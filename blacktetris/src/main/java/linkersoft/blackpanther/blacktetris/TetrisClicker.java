...
public class TetrisClicker extends FrameLayout {
    ...
    private void in(@NonNull Context context, @Nullable AttributeSet attrs){
        ...
        try{
            ...
            String gGlobalWidth=trissa.getString(R.styleable.TetrisClicker_GlobalWidth);

            if(gGlobalWidth==null)gGlobalWidth= util.getScreenWidth(context)+"px";
            if(gGlobalWidth.contains("dp"))GlobalWidth= util.dp2px(Float.parseFloat(gGlobalWidth.split("dp")[0]),context);
            else if(gGlobalWidth.contains("px"))GlobalWidth= Integer.parseInt(gGlobalWidth.split("px")[0]);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
            ...
            mLp.width= Math.round( (width/100F)* GlobalWidth);
            mLp.height= Math.round( (height/100F)* GlobalWidth);
            setLayoutParams(mLp);
